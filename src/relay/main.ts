/**
 * Process entry for the relay: `npm run relay` from `tools/nimvarya`.
 *
 * Binds `127.0.0.1` only. The port defaults to 8766 (distinct from boky's 8765
 * so both relays can run at once) and is overridden by `NIMVARYA_PORT`; a
...
/**
 * Resolve the listen port from a raw `NIMVARYA_PORT` value. Throws (rather
...
export function resolveRelayPort(raw: string | undefined): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 65535) {
    throw new Error(
      `NIMVARYA_PORT must be an integer between 1 and 65535, got "${raw}"`,
    );
  }
  return parsed;
}
...
export async function main(): Promise<void> {
  const port = resolveRelayPort(process.env["NIMVARYA_PORT"]);
  const relay = await startRelay({ host: HOST, port });
  process.stdout.write(
    `nimvarya relay listening on ws://${HOST}:${relay.port}\n`,
  );
